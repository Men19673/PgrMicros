 ...
 * Ultima modificacion: 12 Abril, 2021
 *
 * Created on April 12, 2021, 6:10 PM
 */
#include <xc.h>
#include <stdint.h>
...
void __interrupt()isr(void){
    ...
    INTCONbits.T0IF = 0;        
    PORTA++;                //Incrementar puertoA cada 5ms
    PORTB = 0x00;
    ...
    if (RBIF == 1){
        ...
        if (RB0 == 0){
            flagint = 1;        //Encender flag para luego incrementar
        }
        else{
            if(flagint == 1){
                PORTD++;
        ...
        if (RB1 == 0){
            flagint = 2;         //Encender flag para decrementar 
        }
        else{
            if(flagint == 2){
                PORTD--;
...
void setup(void){
  ...
  ANSEL = 0x00;  //Apagar analogo
  ANSELH = 0x00;
  
  TRISA = 0x00;     //Output     
  TRISB = 0b0000011;//Output excepto por pin 0 y 1    
  TRISC = 0x00;     //Output
  TRISD = 0x00;     //Output
  TRISE = 0x00;     //Output
  
  WPUB = 0b00000011;//Pullup para pines 0 y 1 port B
  IOCB = 0b00000011;//Int on change para pines 0 y 1 port B
